...
<script>
    $(() => {
        ...
        function populateCalendar([firstDay, lastUTCDay], month) {
            ...
            let closingTR = '</tr>'
            let currentDay = ''
            let pendingTemplate = '<span class="pending-alert"><small class="pending-count">1</small></span>'
            ...
            for (let index = firstDay, days = 1; index <= lastUTCDay + firstDay; index++, days++) {
                ...
                if (date.getUTCDate() === days && date.getMonth() === month) {
                    currentDay += '<td class="active"><div><h4>' + days + '</h4>' + pendingTemplate + approvedTemplate + '</div></td>'

                } else {
                    currentDay += '<td class=""><div><h4>' + days + '</h4>' + pendingTemplate + approvedTemplate + '</div></td>'
